...
// Written by Pratipo - MIT license
/*****************************************************************************/

// Constructor for use with hardware SPI (specific clock/data pins):
Pratipo_WS2801::Pratipo_WS2801(uint16_t n, uint8_t order) {
  rgb_order = order;
  alloc(n);
  updatePins();
}
...
Pratipo_WS2801::Pratipo_WS2801(uint16_t n, uint8_t dpin, uint8_t cpin, uint8_t order) {
  rgb_order = order;
  alloc(n);
...
Pratipo_WS2801::Pratipo_WS2801(uint16_t w, uint16_t h, uint8_t dpin, uint8_t cpin, uint8_t order) {
  rgb_order = order;
  alloc(w * h);
  width = w;
  height = h;
...
void Pratipo_WS2801::alloc(uint16_t n) {
  begun   = false;
...
Pratipo_WS2801::Pratipo_WS2801(void) {
  begun     = false;
  numLEDs   = 0;
  pixels    = NULL;
...
void Pratipo_WS2801::begin(void) {
  if(hardwareSPI == true) {
    startSPI();
  } else {
...
void Pratipo_WS2801::updatePins(void) {
  hardwareSPI = true;
  datapin     = clkpin = 0;
...
void Pratipo_WS2801::updatePins(uint8_t dpin, uint8_t cpin) {
...
void Pratipo_WS2801::startSPI(void) {
    SPI.begin();
...
uint16_t Pratipo_WS2801::numPixels(void) {
  return numLEDs;
}

// Change strand length (see notes with empty constructor, above):
void Pratipo_WS2801::updateLength(uint16_t n) {
...
void Pratipo_WS2801::setPixelColor(uint16_t n, uint8_t r, uint8_t g, uint8_t b) {
...
void Pratipo_WS2801::setPixelColor(uint16_t x, uint16_t y, uint8_t r, uint8_t g, uint8_t b) {
...
void Pratipo_WS2801::setPixelColor(uint16_t n, uint32_t c) {
  if(n < numLEDs) { // Arrays are 0-indexed, thus NOT '<='
    uint8_t *p = &pixels[n * 3];
...
void Pratipo_WS2801::setPixelColor(uint16_t x, uint16_t y, uint32_t c) {
  boolean evenRow = ((y % 2) == 0);
  // calculate x offset first
  uint16_t offset = x % width;
  if (!evenRow) {
...
// Set pixel color from 'packed' 32-bit RGB value using x,y coordinate system:
void Pratipo_WS2801::setPixelColor710(uint16_t x, uint16_t y, uint32_t c) {

  int moduleH = x/7;
  int moduleV = y/5;

  uint16_t xx = x - moduleH*7;
  uint16_t yy = y - moduleV*5;

  boolean evenCol = ((xx % 2) == 0);
  // calculate y offset first
  uint16_t offset = yy;
  if (!evenCol) {
    offset = (5-1) - offset; //altura de la columna del modulo, menos 1, substraemos el offset
  }
  // add y offset
  offset += xx * 5;

  // add back module offsets

  offset += moduleH*70 + moduleV*35;

  setPixelColor(offset, c);
}

// Query color from previously-set pixel (returns packed 32-bit RGB value)
uint32_t Pratipo_WS2801::getPixelColor(uint16_t n) {
  if(n < numLEDs) {
    uint16_t ofs = n * 3;
    // To keep the show() loop as simple & fast as possible, the
    // internal color representation is native to different pixel
